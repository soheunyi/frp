...
from dglearn.dg.converter import array2edges


def virtual_refine(operator, initial_support,
                   max_path_len=np.inf, patience=np.inf,
                   max_iter=np.inf, max_n_paths=None,
                   first_ascent=False, refine=True,
                   timeout=np.inf,
                   verbose=0):
    '''
    ...
    '''
    if max_n_paths is None:
        max_n_paths = initial_support.shape[0]**2
    ...
    while virtual_candidate is not None and not timeover():
        ...
        if timeover():
            continue
        for path in find_undirected_paths(
                virtual_support, j, i, max_length=max_path_len, blacklist=[])[
                : max_n_paths]:
            ...
            if len(path) == 1:
                ...
                if virtual_support_path[j, i] == 1 and virtual_support_path[j, i] == 1:
                    ...
                    adj_i = set(np.where(virtual_support_path[i, :])[
                                0].tolist() + np.where(virtual_support_path[:, i])[0].tolist())
                    adj_j = set(np.where(virtual_support_path[j, :])[
                                0].tolist() + np.where(virtual_support_path[:, j])[0].tolist())
        ...
        if best_path_score > 0:
            ...
            best_support, _, _ = hill_climbing(
                operator, initial_support=best_support, first_ascent=first_ascent,
                n_random_restarts=0, max_iter=max_iter, verbose=verbose, timeout=time_left)

            if verbose:
                print("virtualization improved score!", virtual_candidate,
                      "Current best support:", array2edges(best_support))
...
def find_undirected_paths(support, start, end, max_length=np.inf, blacklist=[]):
    ...
    def find_edges(v): return list(set(np.where(support[v, :])[
        0].tolist() + np.where(support[:, v])[0].tolist()))
    ...
    while len(stack) > 0:
        ...
        # dfs
            ...
            if dest not in curr_seen and dest not in blacklist and len(curr_path) < max_length:
                stack.append(
                    [find_edges(dest),
                     dest, curr_path + [(curr_vertex, dest)],
                     curr_seen + [dest]])
