...
def tabu_search(operator, tabu_length, patience,
                initial_support=None, first_ascent=False, refine=True,
                max_iter=np.inf, blacklist=[], max_edges=np.inf,
                tabu_timeout=np.inf, hill_timeout=np.inf,
                verbose=0):
    '''
        Tabu search
            * verbose:
    ...
    while not_improved < patience and n_iter < max_iter and time.time() - start < tabu_timeout:
        delta_score, edit_list, info, move = operator.find_move(
            current_support, first_ascent, tabu_list, blacklist)
        if move is not None:
    ...
    if refine:
        if verbose:
            print("*Refine with greedy steps*")
        return hill_climbing(
            operator, initial_support=best_support, first_ascent=first_ascent, n_random_restarts=0,
            max_iter=max_iter, blacklist=blacklist, verbose=verbose, timeout=hill_timeout)
    else:
