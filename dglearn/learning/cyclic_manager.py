...
class CyclicManager:
    ...
    def __init__(
        ...
        self.n = data.shape[0]
        self.p = data.shape[1]
        self.solver = FactorSolver(data, force_stable=force_stable)
            ...
            for j in range(i + 1, self.p):
                if self.current_support[i, j] == 0 and self.current_support[j, i] == 0:
                    ...
                    if num_edges <= self.max_edges:
                        legal_moves.append(("score_add_edge", i, j))
                    if num_edges <= self.max_edges:
                        legal_moves.append(("score_add_edge", j, i))

                elif (
                    self.current_support[i, j] == 0 and self.current_support[j, i] == 1
                ):
                    ...
                    self.current_support[i, j] == 1 and self.current_support[j, i] == 0
                ):
                    ...
                    self.current_support[i, j] == 1 and self.current_support[j, i] == 1
                ):
    ...
    def find_move(self, support, first_ascent, tabu_list, blacklist):
        ...
        * If no moves found, return (0, [], '')
        ...
        for move in legal_moves:
            ...
            if any([self.is_rev(m, move) for m in tabu_list]):
                continue
            if any([self.is_rev(m, move) for m in blacklist]):
                continue
            ...
            if time.time() - start > self.move_timeout:
                if self.verbose:
                    print(f"Timeout of {self.move_timeout}s reached for a move.")
                break
    ...
    def score_add_edge(self, i, j, man_scc=False):
        ...
        # find original scc
        scc_orig_j = (
            self.find_scc(j) if man_scc else [s for s in self.all_scc if j in s][0]
        )
        ...
        if i in scc_orig_j:
            ...
            if len(scc_new_j) > self.max_cycle:
                ...
                return (
                    ...
                    [(i, j, 1)],
                    stable,
                    "Add edge: %d -> %d, Delta Score: %0.3e" % (i, j, delta_score),
                )
            ...
            if i in scc_new_j:
                ...
                for scc in set(
                    [
                        self.find_scc(k)
                        if man_scc
                        else [s for s in self.all_scc if k in s][0]
                        for k in scc_new_j
                    ]
                ):
                ...
                scc_orig_i = (
                    self.find_scc(i)
                    if man_scc
                    else [s for s in self.all_scc if i in s][0]
                )
    ...
    def score_del_edge(self, i, j, man_scc=False):
        ...
        if i in scc_orig_j:
            ...
            scc_orig_i = (
                self.find_scc(i) if man_scc else [s for s in self.all_scc if i in s][0]
            )
        ...
        return (
            ...
            [(i, j, 0)],
            stable,
            "Del edge: %d -> %d, Delta Score: %0.3e" % (i, j, delta_score),
        )
    ...
    def score_rev_edge(self, i, j, man_scc=False):
        ...
        if len(scc_new_i) > self.max_cycle:
            ...
            return (
                ...
                [(i, j, 0), (j, i, 1)],
                stable,
                "Swap edge: %d -> %d, Delta Score: %0.3e" % (i, j, delta_score),
            )
        ...
        if i in scc_orig_j:
            ...
            if scc_new_i == scc_orig_j:
                # case 1a: i and j remain in same scc
                ll_orig, _ = self.solve_scc(scc_orig_j)
                ll_new, stable = self.solve_scc(
                    scc_orig_j, edit_list=[(i, j, 0), (j, i, 1)]
                )
            ...
            if scc_new_i == scc_orig_i:
                ...
                ll_orig_i, _ = self.solve_scc(scc_orig_i)
                ll_orig_j, _ = self.solve_scc(scc_orig_j)
                ll_new_i, stabi = self.solve_scc(
                    scc_orig_i, edit_list=[(i, j, 0), (j, i, 1)]
                )
                ll_new_j, stabj = self.solve_scc(
                    scc_orig_j, edit_list=[(i, j, 0), (j, i, 1)]
                )
                ...
                ll_new, stable = self.solve_scc(
                    scc_new_i, edit_list=[(i, j, 0), (j, i, 1)]
                )
    ...
    def solve_scc(self, scc_tup, edit_list=[]):
        ...
        # solve
        ll_score, stable = self.solver.solve(
            self.current_support, selected=list(scc_tup)
        )
    ...
    def is_rev(self, move1, move2):
        ...
        if (
            name1 == "score_add_edge"
            and name2 == "score_del_edge"
            and i1 == i2
            and j1 == j2
        ):
            return True
            ...
            name1 == "score_del_edge"
            and name2 == "score_add_edge"
            and i1 == i2
            and j1 == j2
        ):
            return True
